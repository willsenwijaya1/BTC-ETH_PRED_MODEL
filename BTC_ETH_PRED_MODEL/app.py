...
def get_latest_feature_frame():
    ...
    return build_feature_frame(df_eth, df_gold, df_sp500, df_usd)
...
try:
    ...
    st.error(
        "Artifacts model belum ada. Jalankan `train_export_gru.py` dulu supaya file model, scaler, dan metadata tersimpan di folder `artifacts/`."
    )
    st.exception(exc)
    st.stop()
...
with tab1:
    ...
    st.write(
        f"Model membaca window fitur dari **{forecast['latest_window'].index.min().date()}** sampai **{forecast['latest_window'].index.max().date()}**, lalu memprediksi close **{prediction_for_date}**."
    )

    chart_df = df_features[["Eth_Close"]].tail(120).copy()
    future_row = pd.DataFrame({"Eth_Close": [forecast["pred_close_price"]]}, index=[pd.Timestamp(prediction_for_date)])
    plot_df = pd.concat([chart_df, future_row])
